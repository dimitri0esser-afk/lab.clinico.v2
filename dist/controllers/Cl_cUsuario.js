...
export default class Cl_cUsuario {
    ...
    async guardarDatosPaciente() {
        ...
        if (!datos.cedula || !datos.nombre || !datos.whatsapp) {
        ...
        if (existe) {
            // Si existe, cargar sus datos existentes
        ...
        // Mostrar formulario de estudios
    ...
    async guardarEstudios() {
        ...
        if (this.estudiosSeleccionados.length === 0) {
            this.vista.mostrarMensaje("⚠️ Seleccione al menos un examen", "error");
            return;
        }
        ...
        // Preparar paciente con sus estudios
        ...
        if (respuesta.ok) {
            ...
            // Opcional: guardar en endpoint de solicitudes si existe
